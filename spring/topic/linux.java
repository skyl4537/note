...
# shell命令注意点: 1).区分大小写; cd和CD表示不同指令. 2).指令太长,可使用进行换行处理

# 输入/输出重定向
	#command > file		->	将输出重定向到 file。
	#command >> file	->	将输出 [追加] 到 file。
...
#sort: 将文件内容进行排序, 并将排序结果标准输出
	#-r: 以相反的规则进行排序	-u: 忽略相同行		-n: 依数值大小进行排序
	#-t<分隔字符>: 指定排序时所用的栏位分隔字符		+<起始栏位>-<结束栏位>: 以指定的栏位来排序,范围由起始栏位到结束栏位的前一栏位
	
	sort (-r) file	//以正序(倒序)方式对file内容排序
...
'常用命令': http://www.runoob.com/w3cnote/linux-useful-command.html
	...
	touch testfile		//创建文件 或 刷新文件的时间属性
	rm -rf /var/log		//删除目录及其子目录; "-r:递归删除; -f:强制删除,不予提示"
	mv err file			//重命名文件
