...
#创建表,插入数据
	...
	desc flower; //查看表的详细信息
	
	INSERT INTO flower VALUES(default,'牵牛花',1.1,'山西'); //自增主键传值default
...
#命令行操作
	//-h: 服务器ip; -P: 端口号(默认3306,修改后必须显示指定端口); -u: 用户名; -p: 密码
...
#linux下mysql日志
	//linux查找这两个文件所在位置命令:  find / -name my.cnf(mysql.cnf)
	log-error=/var/log/mysql.log	//在 my.cnf 或 mysql.cnf 中进行配置
...
#常用函数
	...
	0.Mysql时间加减函数为date_add() date_sub()
	SET @now=NOW(); SELECT DATE_ADD(@now,INTERVAL 1 DAY)	//1天以后的时间
